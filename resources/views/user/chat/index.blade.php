                    ...
                    <div class="position-relative">
                        <div class="chat-messages p-4" id="chat-section">
                            ...
                            @if($v->from_user_uuid == Auth::user()->uuid)
                                ...
                                @else
                                    <div class="chat-message-left pb-4" data-uuid="{{$v->fromUser->fromChats()->orderBy('created_at', 'desc')->first()->uuid}}">
                                        <div>
                                            <img src="https://bootdey.com/img/Content/avatar/avatar3.png" class="rounded-circle mr-1" alt="Sharon Lessman" width="40" height="40">
                                            <div class="text-muted small text-nowrap mt-2">{{$v->created_at}}</div>
                                        </div>
...
<script type="text/javascript">
    function fetchMessages() {
        $.get('/chat/get-last-chat-from-admin', function (data) {
            ...
            var lastMessageUuid = $('#chat-section').children().last().attr('data-uuid');
            ...
            if (lastMessageUuid === lastUuid){
                ...
                $('#chat-section').append('<div class="chat-message-left pb-4"><div><img src="https://bootdey.com/img/Content/avatar/avatar3.png" class="rounded-circle mr-1" alt="Sharon Lessman" width="40" height="40"><div class="text-muted small text-nowrap mt-2">'+data.data.created_at+'</div></div><div class="flex-shrink-1 bg-light rounded py-2 px-3 ml-3"><div class="font-weight-bold mb-1">'+"arief keren"+'</div>'+data.data.message+'</div></div>');
            }
            });
    }
